...
const whatsappLinkGenerator = (phoneNumber, productTitle, productQuantity, buyerName, buyerAdress, buyerPayment) => `https://api.whatsapp.com/send?phone=${phoneNumber}&text=Olá eu quero: ${productQuantity} ${productTitle} - Entregar para ${buyerName} no endereco: ${buyerAdress} - A forma de pagamento será: ${buyerPayment} `
...
const checkout = phoneNumber => {
    ...
    form.addEventListener('submit', e => {
        ...
        const buyerAdress = form.querySelector('input#adress').value
        const buyerPayment = form.querySelector('select#payment').value


        const whatsappUrl = whatsappLinkGenerator(phoneNumber, productTitle, productQuantity, buyerName, buyerAdress, buyerPayment)
